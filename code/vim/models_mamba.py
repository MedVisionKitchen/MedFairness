...
class MaeEncoder(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False,
        ...
        if not self.if_bidirectional:
            ...
            for index, layer in enumerate(self.layers):
                ...
                if index == 5 or index == 7:
                    hidden_states ,  lossChannelT = InstanceWhiteningLoss(hidden_states)
                    lossChannel += lossChannelT
        ...
        return hidden_states, lossChannel

    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False,
                if_random_token_rank=False):
        x,lossC = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position,
                                  if_random_token_rank=if_random_token_rank)
        return x, lossC


class InstanceWhiteningLoss(torch.nn.Module):
    def __init__(self):
        super(InstanceWhiteningLoss, self).__init__()
        self.eps = 1e-5
    ...
    def instance_whitening_loss(self, x):
        ...
        HW = H * W
        eye = torch.eye(C).cuda()  # Identity matrix for regularization
        ...
        f_cor_masked = torch.triu(f_cor)

        # Compute the off-diagonal sum of the upper triangular part
        off_diag_sum = torch.sum(torch.abs(f_cor_masked), dim=(1, 2), keepdim=True)  # B X 1 X 1

        # Compute the loss and average over the batch
        loss = torch.clamp(off_diag_sum, min=0)  # B X 1 X 1
        loss = torch.sum(loss) / B

        return x, loss

    def forward(self, x):
        return self.instance_whitening_loss(x)
